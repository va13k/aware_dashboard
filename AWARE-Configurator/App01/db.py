...
def check_insert_privileges(ip, port, database, username, password):
    ...
    if db is None:
        result["msg"] = (
            "Failed to create connection to database, please check if credentials are correct."
        )
        return result
    ...
    try:
        ...
        cursor.close()
        if (
            re.search("ALL|CREATE|DROP|EXECUTE|UPDATE|ROUTINE|EVENT|TRIGGER", privileges)
            is not None
        ):
            result["msg"] = (
            ...
            result["msg"] = (
                "Insufficient privileges for this MySQL account. "
...
def check_root_privileges(ip, port, database, username, password):
    ...
    try:
        ...
        if re.search("ALL", privileges) is not None:
            ...
            re.search("CREATE", privileges) is not None
...
def init_database(
    ip, port, database, root_username, root_password, insert_username, insert_password, require_ssl
):
        ...
        if require_ssl:
            createUserSql = """
             ...
             flush privileges;""".format(
                insertUsername=insert_username, insertPassword=insert_password, database=database
            )
        else:
            createUserSql = """
                CREATE USER IF NOT EXISTS '{insertUsername}'@'localhost' IDENTIFIED BY '{insertPassword}';
