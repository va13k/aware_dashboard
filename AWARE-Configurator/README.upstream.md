...
> **This is upstream's document, kept for provenance. Its instructions do not apply
> to this repository, and one of them is destructive here.**
>
> It describes the Configurator as a standalone Django application installed onto a
> host: pip and npm run by hand, gunicorn started by hand, and nginx configured by
> `preparation.sh`. In this repository the Configurator is a container that
> `docker compose` builds and runs, and `preparation.sh` is not part of that. It
> deletes files under `/etc/nginx/sites-available` and restarts the host's nginx, so
> running it on a machine hosting this deployment takes the deployment down.
>
> For this fork, read [README.md](README.md).
