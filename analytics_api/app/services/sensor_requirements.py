...
ANDROID_SETTING_STREAMS: dict[str, tuple[str, ...]] = {
    ...
    "status_plugin_esm_scheduler": ("esms",),
    "status_plugin_openweather": ("plugin-openweather",),
    # `processor` and `screenshot` have tables in the Android schema and no API
    # route. The dashboard's processor card is served by iOS data only.
    "status_processor": NO_STREAM,
    "status_screenshot": NO_STREAM,
...
IOS_SETTING_STREAMS: dict[str, tuple[str, ...]] = {
    ...
    "status_plugin_esm_scheduler": ("calendar-esm-scheduler",),
