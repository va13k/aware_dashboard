...
def prepared(watermarks=None):
    """A rewriter that has already read the accelerometer definition."""
...
def test_replace_mode_passes_every_line_through():
    ...
    lines = [b"DROP TABLE IF EXISTS `accelerometer`;\n", *CREATE]
...
def test_merge_keeps_the_stored_table_and_creates_a_missing_one():
    ...
    output = run(rewriter, [b"DROP TABLE IF EXISTS `accelerometer`;\n", *CREATE])
    assert b"DROP TABLE" not in output
    assert b"CREATE TABLE IF NOT EXISTS `accelerometer`" in output


def test_merge_drops_the_id_column_so_rows_take_fresh_identity():
    rewriter = prepared()
    insert = b"INSERT INTO `accelerometer` VALUES (1,100,'phone-a',0.5,NULL);\n"
    output = rewriter.feed(insert)
    assert output == (
        b"INSERT INTO `accelerometer` "
        b"(`timestamp`,`device_id`,`double_values_0`,`label`) "
        b"VALUES (100,'phone-a',0.5,NULL);\n"
    )


def test_merge_admits_only_rows_newer_than_the_device_watermark():
    rewriter = prepared({("aware_android", "accelerometer"): {"phone-a": 200.0}})
    insert = (
        b"INSERT INTO `accelerometer` VALUES "
...
def test_a_fully_stored_statement_contributes_nothing():
    rewriter = prepared({("aware_android", "accelerometer"): {"phone-a": 500.0}})
    insert = b"INSERT INTO `accelerometer` VALUES (1,100,'phone-a',0.5,NULL);\n"
...
def test_row_tallies_report_what_was_added_and_skipped():
    ...
    rewriter = DumpRewriter(
        MERGE,
        {("aware_android", "accelerometer"): {"phone-a": 200.0}},
...
def test_the_count_cache_is_left_out_of_a_merge():
    ...
    lines = [
        ...
        b") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;\n",
        b"INSERT INTO `record_counts` VALUES ('accelerometer','phone-a',5,9,100);\n",
    ]
...
def test_watermarks_are_scoped_to_the_database_in_use():
    """Both platform databases have an `accelerometer`, so the table name alone
    cannot pick the right watermark."""
    marks = {
        ("aware_android", "accelerometer"): {"phone-a": 500.0},
        ("aware_ios", "accelerometer"): {"phone-a": 10.0},
    }
    rewriter = DumpRewriter(MERGE, marks)
    run(rewriter, [b"USE `aware_ios`;\n", *CREATE])
    output = rewriter.feed(
        b"INSERT INTO `accelerometer` VALUES (1,100,'phone-a',0.5,NULL);\n"
    )
...
def test_an_explicit_column_list_on_the_insert_is_respected():
    rewriter = DumpRewriter(MERGE, {("aware_android", "accelerometer"): {"phone-a": 50.0}})
    rewriter.feed(b"USE `aware_android`;\n")
    insert = (
        b"INSERT INTO `accelerometer` (`_id`,`timestamp`,`device_id`) "
        b"VALUES (1,100,'phone-a');\n"
    )
    output = rewriter.feed(insert)
    assert output == (
        b"INSERT INTO `accelerometer` (`timestamp`,`device_id`) "
