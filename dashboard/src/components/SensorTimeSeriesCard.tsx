...
export default function SensorTimeSeriesCard({
  config,
  records,
  loading,
}: Props) {
  ...
  return (
    <div className="bg-card backdrop-blur-xl border border-wire rounded-3xl shadow-card p-5">
      ...
      {values.length > 0 && (
        <div className="flex items-center gap-3 mb-3 text-[11px] text-sage">
          ...
          {isEnum ? (
            <span>
              last{" "}
              <b className="text-ink">{enumLabels![lastVal!] ?? lastVal}</b>
            </span>
          ) : (
            <>
              <span>
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={isEnum ? 180 : 160}>
          <LineChart
            data={data}
          ...
          >
            ...
            {isEnum ? (
              <YAxis
                ...
                width={90}
                ticks={enumKeys}
                domain={[enumKeys[0], enumKeys[enumKeys.length - 1]]}
                tickFormatter={(v) => enumLabels![v as number] ?? String(v)}
              />
            ) : (
            ...
            <Tooltip
              ...
              formatter={
                isEnum
                  ? (v: unknown) => [
                      enumLabels![v as number] ?? String(v),
                      config.label,
                    ]
                  : undefined
              }
