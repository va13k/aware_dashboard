...
function latestPoint(points: AxisPoint[]): AxisPoint | null {
  ...
  return (
    <div className="bg-card backdrop-blur-xl border border-wire rounded-3xl shadow-card p-5">
      <div className="flex items-center gap-2 mb-1">
        <span className="w-2 h-2 rounded-full shrink-0 bg-[#84cc16]" />
      ...
      {data.length > 0 && last && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-3 text-[11px] text-sage">
          ...
          <span>
            |a| <b className="text-ink">{fmt(last.magnitude, 3)}</b>
          </span>
          {last.accuracy != null && (
            <span>
              accuracy <b className="text-ink">{fmt(last.accuracy, 0)}</b>
            </span>
          )}
          {labels.length > 0 && (
            <span>
              labels <b className="text-ink">{labels.length}</b>
            </span>
          )}
        </div>
      )}
