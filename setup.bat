...
REM Create a run marker so we can detect files generated by this wizard session
type nul > .setup-session.start
...
powershell -NoProfile -Command ^
  "$marker = Get-Item '.setup-session.start' -ErrorAction SilentlyContinue; " ^
  "if ($null -eq $marker) { exit 1 }; " ^
  "$targets = @('.env', 'aware-micro-server\aware-config.json', 'studies\studyConfig.json', 'studies\index.html'); " ^
  "$items = foreach ($path in $targets) { Get-Item $path -ErrorAction SilentlyContinue }; " ^
  "if ($items.Count -ne $targets.Count) { exit 1 }; " ^
  "$isReady = ($items | Where-Object { $_.LastWriteTimeUtc -gt $marker.LastWriteTimeUtc }).Count -eq $targets.Count; " ^
  "if ($isReady) { exit 0 } else { exit 1 }" >nul
if errorlevel 1 goto WAIT_LOOP
del .setup-session.start
