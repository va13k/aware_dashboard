...
[NOTE]
====
This is upstream's document, kept as it was. It describes AWARE Micro as a
standalone server installed on a host with Java, which is not how this repository
deploys it: here it is a container built by `docker compose`, run twice, configured
from a file the deploy writes, and carrying rules upstream does not have.

For this fork, read link:README.md[README.md] instead.
====
...
```
git clone https://github.com/awareframework/aware-micro-server.git
```
